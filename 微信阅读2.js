...
微信阅读2
需要青龙环境
入口，微信打开 -> 入口1：https://api.sulaixian.top:10269/yunonline/v1/auth/3f74b41967147388a481d53dbf152c12?codeurl=sulaixian.top:10269&codeuserid=2&time=1678542914
                入口2：https://api.lopinc.top:10263/yunonline/v1/auth/3f74b41967147388a481d53dbf152c12?codeurl=lopinc.top:10263&codeuserid=1&time=1678542914
...
class UserInfo {
    ...
    async dotask() {
        try {
            ...
            /*
            "1" == result.info.type && 1 !== this.dx && (console.log(`文章获取成功 `), this.b = 1, await $.wait(2000), await this.readfinish()),
                "3" == result.info.type && 1 !== this.dx && (console.log(`已限制阅读   尝试过验证`), this.x = result.info.key, this.c = result.info.url.split("/s/")[1], this.b = 2,
                    await $.wait(8000), await this.readfinish());
                    */
        } catch (e) {
            console.log(e)
        } finally {
    ...
    async jump(readurl) {
        try {
            ...
            var sj = Math.random() * (9000 - 6000) + 5000
            ...
            let result = httpResult;
            //console.log(result)
            //var sj = Math.random() * (9000 - 6000) + 10000
            //console.log('等待:'+ sj)
            await $.wait(sj)
            await this.add_gold()
    ...
    async add_gold() {
        try {
             var sj = Math.random() * (9000 - 6000) + 2000
