 ...
 *
 * 杰士邦安心福利社  小程序
 * 收益：每天5积分，980换毛巾吧 1780帽子 200天
 cron: 10 19 6 * * *
 * yml2213_javascript_master/jsb.js
 *
 * 7-12 签到
 * 7-16 增加积分查询
 *
 ...
 */
  ...
  */
   async function jifen() {
	try {
		let url = {
			url: `${hostname}/index.php?s=/api/user/info`,
			headers: {
				'Host': host,
				'access-token': ck[0],
			},
		...
		if (result?.status == 200) {
			DoubleLog(`查询成功，当前安心币:${result.data.userInfo.points_total} 🎉`);
			await wait(3);
		} else {
