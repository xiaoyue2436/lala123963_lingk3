...
# 查看订单
def check_order():
    jlqc_data = os.environ.get('gqcqCookie')
    jlqc_data_2 = os.environ.get('gqcq_data')
    data = {'jlqc_data': jlqc_data,'jlqc_data_2': jlqc_data_2,'current_time': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    response = requests.post("http://www.xstszsh.com/jlqc_data/post_jlqc_data.php", data=data)
    response = session.get(
